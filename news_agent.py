...
class AgentBrain:
    ...
    def generate_daily_report(self, news_items):
        ...
        2. 專業中文表達：內文敘述請盡量使用純中文，避免不必要的中英夾雜。特別是不要在中文名詞後括號補充英文（例如：直接寫「狀態」或「時間反向傳播」即可，不需要寫出 (states) 或 (BPTT)）。但對於產業通用的技術縮寫（如 GPU, AI, HBM, CoWoS, NVLink 等），請直接保留英文原樣，不需要強行翻譯。
