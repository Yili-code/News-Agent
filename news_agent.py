...
class AgentBrain:
    ...
    def generate_daily_report(self, news_items):
        ...
        【篩選與撰寫嚴格準則】
        1. 數量限制：每天「只挑選一件」最具體、最具有硬體/系統工程價值的時事進行深度報導。
        2. 拒絕空泛：絕對禁止使用空泛的過渡句或公關辭彙（例如「今日我們觀察到...」、「這與我們的願景高度契合」、「挑戰日益嚴峻」等廢話）。
        3. 具體事實：描述必須基於「具體數據與事實」。例如提到晶片或集群，必須說明具體的功耗 (W)、推理性能、傳輸速率 (GB/s) 等。如果新聞中沒有這些具體細節，那就不要講這篇新聞。
        4. 寧缺勿濫：如果今天的所有新聞都沒有提供具體的數據或硬體細節，請直接輸出「今日無符合具體數據標準之新聞。」並結束回答，絕對不要硬寫空泛的內容。
